...
const renderpeopleList = (people) => {
    people.map((person) => {
        ...
        li.classList.add('text-lg', 'cursor-pointer', 'text-green-500', 'hover:text-rose-500', 'font-medium')
...
const displayPersonInfo = (person) => {
    ...
    <h2 class = 'font-bold mb-2 text-2xl'>${person.name}</h2>
    <p>${person?.contact?.phone || 'N/A'}</p>
    <p>${person?.address?.city || 'N/A'}</p>
    <p>${person?.hobbies?.map(a => a).join(' / ') || 'N/A'}</p>
    `
}
